...
def create_q_model(n_classes, n_actions):
    ...
    # feat = K.layers.Lambda(lambda x: x[:, 0:n_classes - n_actions + 1])(inputs)     # 截取输入的第0列到c-a+1列
    ...
    for episode in range(Flags.n_episodes):
        ...
        while not done:
            try:
                ...
                # print("revised_state维度：",len(revised_state))
                ...
                if iter_count % update_after_actions == 0 and len(done_history) > 100 and min_count >= 10:
                    ...
                    # 从done_history列表的索引范围中随机选择100个索引
                    print("状态维度：", len(state_history))
                    # 优先级采样
                    ...
                    index_action =[(index,action)for index,action in enumerate(action_history)]
                    ...
                    for a, indices in action_indices.items():
                        ...
                        # if(min_count <= 10):
                        #     print("采样索引号为", s_indices[a])
                        #     print("经验池：",np.array(state_history).shape,np.array(state_next_sample).shape,np.array(rewards_history).shape,np.array(action_history).shape)
                        #     print("最少数量的样本种类数量n为：", min_count)
                        #     print("随机取出的样本数：",np.array(state_sample).shape,np.array(state_next_sample).shape,np.array(rewards_sample).shape,np.array(action_sample).shape)
        ...
        # print('The number of not ransdom actions was ', n_not_random)
