...
class Agent:
    def __init__(self, dataset, config):
        ...
        tf_config = tf.compat.v1.ConfigProto()      # TensorFlow配置
        self.sess = tf.compat.v1.Session(config=tf_config)      # 创建一个TensorFlow会话
        self.acc = []
    ...
    def get_action(self, state, is_train=True):
        if np.random.random() < self.epsilon and is_train:
            ...
            q = self.sess.run(self.net.q_mnet, feed_dict={self.net.state: state})       # 获取网络输出的Q值
    ...
    def update_epsilon(self, train_step):
        self.epsilon = np.clip(
            ...
            *[self.epsilon_end, self.epsilon_start])        # 使用线性衰减更新epsilon，并确保其值在epsilon_end和epsilon_start之间
    ...
    def train(self):
        metric_names = ['recall', 'specificity', 'ppv', 'npv', 'auroc']     # 定义评估指标名称
        results_df = pd.DataFrame(np.zeros((len(metric_names), 1)), index=metric_names) # 初始化训练结果DataFrame
        results_df_v = pd.DataFrame(np.zeros((len(metric_names), 1)), index=metric_names) # 初始化验证结果DataFrame
        results_df_t = pd.DataFrame(np.zeros((len(metric_names), 1)), index=metric_names) # 初始化测试结果DataFrame
        # 初始化TensorFlow变量和目标网络
        ...
        while train_step < self.training_steps:
            # 循环进行训练
            a, a_raw = self.get_action(s_array[np.newaxis, ...])             # 获取动作和原始动作概率
            r, t = self.get_reward(y, a)         # 获取奖励和是否结束训练的标志
            # 随机选一个状态开始
            ...
            if self.memory.size >= 5000:
                ...
                self.sess.run(self.net.train_opt, feed_dict={self.net.state: sample_s, self.net.action: sample_a,
                                                             self.net.reward: sample_r,
                                                             self.net.terminate_training: sample_t,
                ...
                label_list.append(y)
                # print("预测动作",pred_list)
                # print("预测概率",pred_list_raw)
                ...
                if train_step % self.config.show_progress == 0:
                    ...
                    recall, specificity, recall_test, specificity_test, ppv_test, npv_test, auc_score = self.evaluate(
                        label_list, pred_list, eval_y_list, eval_a_list, pred_list_raw, train_step,
                        train_or_test='Train')
                    results_df = results_df.append(
                        {'recall': recall_test, 'specificity': specificity_test, 'ppv': ppv_test, 'npv': npv_test,
                         'auroc': auc_score}, ignore_index=True)

                    print("Validation Scores")
                    recall_v, specificity_v, recall_test_v, specificity_test_v, ppv_test_v, npv_test_v, auc_score_v = self.evaluate(
                        label_list, pred_list, eval_y_list, eval_a_list, eval_a_raw_list, train_step,
                        train_or_test='Validation')
                    results_df_v = results_df_v.append(
                        {'recall': recall_test_v, 'specificity': specificity_test_v, 'ppv': ppv_test_v,
                         'npv': npv_test_v, 'auroc': auc_score_v}, ignore_index=True)
    ...
    def calc_metrics(self, y, pred, pred_raw):
        ...
        print('Beat_Accuracy: {:.3f}'.format(max(self.acc)))
    ...
    def evaluate(self, train_label, train_prediction, val_label, val_prediction, pred_raw, step, train_or_test="Train"):
        ...
        for idx, (label, prediction) in enumerate(zip(labels, predictions)):
            recall, specificity, recall_score, specificity_score, ppv_score, npv_score, auc_score = self.calc_metrics(
                label, prediction, pred_raw)
            print("\n")
